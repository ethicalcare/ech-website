...
`site/sitemap.xml` is the checked canonical route inventory. On an approved public launch, the build creates the production sitemap from the actual output pages, without guessed modification dates. Enable `indexing` in `site.config.json` only after completing the launch checklist. Cloudflare production builds on `main` then generate indexable pages and the production sitemap. Local builds remain nonindexable unless explicitly run with `SITE_BUILD_MODE=production`. Generated `_headers` keep both the project and deployment-specific Pages hostnames nonindexable, including when the company domain is public. `_redirects` preserves the mapped legacy business URLs. Confirm these controls through hosted HTTP responses after deployment. Resource guides use Ethical Care at Home as their organizational author. Record their actual first-publication dates at launch; do not invent personal reviewers or stagger dates artificially. Dates remain fixed on later builds unless an actual article update warrants a modification date. Preview builds retain page-level `noindex` even when public indexing is enabled.
